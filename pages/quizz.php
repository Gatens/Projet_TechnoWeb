...
function quizzName ($ID_quizz,$question_number,$value,$goodQuestion,$bdd){
...
function displayQuizz($ID_quizz,$bdd){
    ...
    echo('<form action=index.php?page=answer&id='.$ID_quizz.' method="post"><div id="questionContent">');
    ...
    foreach ($question as $key=>$value){
        ...
        if($value['question_input_type']=='selection'){
            ...
            echo(" <select  name='Question".$question_number."Quizz".$value['question_quizz_id']."' class='selection' form='selection'>");
        ...
        if($value['question_input_type']=='radio'){
            ...
            $answer = $bdd->query('SELECT answer_id,answer_text,is_valid_answer FROM answer WHERE answer.answer_question_id ='.$value['question_id'])->fetchAll();
            foreach ($answer as $key2 => $proposition){
                echo('<input type="radio" class="radioElmnt" name="radio" value='.$proposition['answer_id'].' class="radio"> <label for="radio">'.$proposition['answer_text'].'</label> <br/>');
            }
            echo('</div>');
        }
        ...
        if($value['question_input_type']=='checkbox'){
            ...
            foreach ($answer as $key2 => $proposition){
                ...
                echo("<div> <input class='checkboxElmnt' type='checkbox' id='rep".$answer_counter."1q1' name='choix[]'' value=".$proposition['answer_id']." '> <label for='rep1q1'>".$proposition['answer_text']."</label></div>");
            }
            echo('</div>');
            $answer_counter=0;
        }
